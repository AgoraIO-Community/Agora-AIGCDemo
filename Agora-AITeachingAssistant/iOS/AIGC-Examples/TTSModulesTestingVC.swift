//
//  TTSModulesTestingVC.swift
//  AIGC-Examples
//
//  Created by ZYP on 2023/11/24.
//
...
class TTSModulesTestingVC: UIViewController, AgoraAIGCServiceDelegate, RtcManagerDelegate, MainViewDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        title = "Integration:" + sttProviderName + "+" + llmProviderName + "+" + ttsProviderName
        view.backgroundColor = .white
        view.addSubview(mainView)
        mainView.frame = view.bounds
        mainView.delegate = self
        mainView.showTextField(show: true)
        initRtc()
        initAIGC()
    }
    ...
    func mainViewDidShouldSendText(text: String) {
        let roundId = UInt8.random(in: 0...UInt8.max)
        service.interrupt(.TTS)
        service.pushTxt(toTTS: text, roundId: nil)
        let info = LLMModulesTestingView.Info(uuid: "llm" + "\(roundId)", content: text)
        mainView.addOrUpdateInfo(info: info)
    }
